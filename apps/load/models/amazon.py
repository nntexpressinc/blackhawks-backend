...
import logging
from apps.load.models import Load  # Import Load model from your existing models
...
def process_excel_file(payment_instance):
    ...
    try:
        ...
        df['Gross Pay'] = df['Gross Pay'].str.replace('$', '').str.replace(',', '').astype(float)
...
def find_and_update_load(trip_id, load_id, gross_pay):
    ...
    try:
        ...
        if trip_id and pd.notna(trip_id):
            try:
                matched_load = Load.objects.get(reference_id=trip_id)
        ...
        if not matched_load and load_id and pd.notna(load_id):
            try:
                matched_load = Load.objects.get(reference_id=load_id)
            except Load.DoesNotExist:
                pass
